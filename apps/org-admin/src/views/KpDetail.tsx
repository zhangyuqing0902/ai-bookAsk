...
import { Search, Dropdown, DataGrid, TextInput, Modal, ConfirmDialog, pickFile, ACCEPT, type Col } from '@aba/ui-admin';
...
// 6.3:文件名前彩色 icon(pdf 红 / word 蓝 / 图音视 珊瑚红,视觉风格一致)
...
const fileIcon = (icon: string) =>
  // 4.5:PDF 文件 icon 中间叠加「PDF」三个小字母,一眼可辨
  icon === 'i-file' ? (
    <span className="pdf-ic">
      <Icon id={icon} w={16} h={18} style={{ color: ICON_COLOR[icon] }} />
      <span className="pdf-ic-tx">PDF</span>
    </span>
  ) : (
    <Icon id={icon} w={15} h={15} style={{ verticalAlign: -3, color: ICON_COLOR[icon] ?? 'var(--ink-3)' }} />
  );
...
const inferKind = (name: string): { icon: string; type: string } => {
  ...
  if (['mp4', 'mov', 'avi', 'mkv', 'webm'].includes(ext)) return { icon: 'i-play', type: '视频' };
...
const KB0: KbFile[] = [
  ...
  { id: 6, name: '手术演示.mp4', icon: 'i-play', type: '视频', slice: '—', st: { kind: 'fail', text: '向量化失败', reason: '文件解析失败：视频时长超过 30 分钟上限,请压缩后重传' } },
];
        ...
        <div className="fm-card">
          ...
          <div className="radio-list" style={{ padding: '6px 0 16px' }}>
            ...
            <div className={'radio-opt' + (price === 1 ? ' on' : '')} onClick={() => setPrice(1)}>
              <div className="rd" />
              {/* 6.10:文案 */}
              <div><div className="rt">会员</div><div className="rs">会员专享媒体资源，永享内容单独计价；文字内容全量免费浏览。</div></div>
            </div>
          </div>
        </div>
      )}
