...
import { LineChart, RangePicker, InfoDot, exportWorkbook, fmtCn, UNIT_NOTE } from '@aba/ui-admin';
...
const RETENTION: Record<string, { rows: Bar[]; sample: string; cutoff: string }> = {
  '7 日': {
    rows: [{ nm: '次日', pct: 42, color: I, pv: '42%' }, { nm: '7 日', pct: 25, color: J, pv: '25%' }, { nm: '30 日', pct: 0, color: A, pv: '—' }],
    sample: '1,126 人', cutoff: '次日截至 07-10；7 日截至 07-04；30 日样本尚未成熟',
  },
  '30 日': {
    rows: [{ nm: '次日', pct: 44, color: I, pv: '44%' }, { nm: '7 日', pct: 27, color: J, pv: '27%' }, { nm: '30 日', pct: 17, color: A, pv: '17%' }],
    sample: '4,782 人', cutoff: '次日截至 07-10；7 日截至 07-04；30 日截至 06-11',
  },
};
...
function Kpi({ lab, val, unit, suf, delta, info, periodDays }: { lab: string; val: string; unit?: string; suf?: string; delta?: string; info: string; periodDays?: number }) {
    ...
    <div className="kpi">
      ...
      {delta && (
        <div className="delta up">
          ...
          {delta}{periodDays && <span className="period-compare">{comparisonPeriodLabel(periodDays)}</span>}
        </div>
      )}
      {!delta && periodDays && <div className="delta">上一周期 <span className="period-compare">{comparisonPeriodLabel(periodDays)}</span></div>}
    </div>
  );
}
...
export function DataBoard() {
  ...
  const [retentionRange, setRetentionRange] = useState('7 日');
  const d = RANGE[rangeLabel] ?? RANGE['7 日'];
  const retention = RETENTION[retentionRange] ?? RETENTION['7 日'];
    ...
    <>
      <div className="page-head">
        ...
        <div className="pa">
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => { void exportWorkbook('机构数据看板', [{ name: '区间指标', title: '机构数据看板 · 区间指标', subtitle: `当前筛选=${rangeLabel} · ${comparisonPeriodLabel(periodDays)}`, headers: ['主题', '指标', '数值', '统计说明'], rows: [['用户', '新增用户', d.newUsers, '区间首次注册'], ['提问', '总提问', d.totalAsk, '含追问'], ['提问', '人均提问', d.perUser, '提问/活跃用户'], ['提问', '平均会话轮次', d.rounds, '提问/会话'], ['营收', 'GMV', d.gmv, '区间已支付'], ['营收', '付费用户', d.payUsers, '区间去重'], ['营收', '退款金额', d.refundAmt, '区间成功退款'], ['营收', '净 GMV', d.netGmv, 'GMV-退款']], widths: [16, 24, 18, 40] }, { name: '留存率', title: '用户留存率', subtitle: `独立队列筛选=${retentionRange} · 成熟样本=${retention.sample} · ${retention.cutoff}`, headers: ['留存节点', '留存率', '成熟口径'], rows: retention.rows.map((r) => [r.nm, r.pv, retention.cutoff]), widths: [18, 16, 54] }, { name: '提问趋势', title: `提问量趋势 · ${rangeLabel}`, headers: ['时间', '提问量'], rows: d.askTrend.x.map((x, i) => [x, d.askTrend.v[i]]), widths: [20, 18] }]); toast('正在生成 XLSX'); }}>
      ...
      {tab === 0 && (
        <>
          {/* 实时活跃概览：DAU/WAU/MAU 为固定窗口快照，切换顶部时间筛选时纹丝不动，
              故与随区间联动的指标分区，加「实时」标签消除「切了区间前三张卡不动」的困惑 */}
          ...
          {/* 区间分析：以下指标随顶部时间筛选（今日 / 7 日 / 30 日）联动 */}
          <div className="dash-section-title" style={{ marginTop: 22 }}>
            区间分析
            <span className="dash-section-sub">· {rangeLabel} · 指标随顶部时间筛选联动（留存率除外，见其独立队列口径）</span>
          </div>
          <div className="kpi-row" style={{ gridTemplateColumns: 'repeat(4,1fr)' }}>
            <Kpi lab="新增用户" val={d.newUsers} suf="人" delta={d.newDelta} periodDays={periodDays} info="所选区间内首次注册的用户数，按用户 ID 精确去重。" />
          </div>
          <div className="grid2" style={{ marginTop: 16 }}>
            <div className="chart-card" style={{ margin: 0 }}>
              <div className="chart-head">
                <CardTitle t="留存率" info="按注册日形成独立新增用户队列，只纳入已完整经过对应观察天数的成熟队列；今日不计算留存并显示“—”。留存周期不跟随页面全局筛选。" />
                <RangePicker presets={['7 日', '30 日']} defaultActive={0} onChange={(r) => setRetentionRange(r.label)} />
              </div>
              <Bars data={retention.rows} />
              <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 12 }}>成熟样本：{retention.sample} · 成熟截止：{retention.cutoff}</div>
            </div>
            <div className="chart-card" style={{ margin: 0 }}>
      ...
      {tab === 1 && (
        <>
          <div className="kpi-row" style={{ gridTemplateColumns: 'repeat(5,1fr)' }}>
            <Kpi lab="总提问" val={d.totalAsk} suf="条" info="所选区间内 C 端提问条数（含追问）。" periodDays={periodDays} />
            <Kpi lab="人均提问" val={d.perUser} suf="条/人" info="所选区间总提问 ÷ 同区间活跃用户数。" periodDays={periodDays} />
            <Kpi lab="平均会话轮次" val={d.rounds} suf="轮" info="所选区间总提问 ÷ 总会话数，衡量对话深度。" periodDays={periodDays} />
            <Kpi lab="答案点赞率" val={d.likeRate} info="所选区间点赞答案数 ÷ 已完成答案数；明细见答案反馈。" periodDays={periodDays} />
            <Kpi lab="答案反馈率" val={d.fbRate} info="所选区间收到用户反馈（点踩 / 举报 / 建议）的答案数 ÷ 已完成答案数；明细见答案反馈工作台。" periodDays={periodDays} />
          </div>
      ...
      {tab === 2 && (
        <>
          ...
          <div className="kpi-row" style={{ gridTemplateColumns: 'repeat(5,1fr)' }}>
            <Kpi lab="区间 GMV" val={d.gmv} info="所选区间内已支付订单金额合计（会员 + 永享）。" periodDays={periodDays} />
            <Kpi lab="付费用户" val={d.payUsers} suf="人" info="所选区间内产生有效支付的去重用户数。" periodDays={periodDays} />
            <Kpi lab="付费转化率" val={d.payRate} info="区间付费用户 ÷ 区间活跃用户。" periodDays={periodDays} />
            <Kpi lab="ARPPU（每付费用户均收入）" val={d.arppu} info="区间支付收入 ÷ 区间付费用户数。" periodDays={periodDays} />
            <Kpi lab="续费率" val={d.renew} info="所选区间内到期且完成续费的会员数 ÷ 同区间到期会员数。" periodDays={periodDays} />
          </div>
          {/* 小节② 退款 */}
          <div className="dash-section-title" style={{ marginTop: 22 }}>
            退款
          ...
          <div className="kpi-row">
            <Kpi lab="退款金额" val={d.refundAmt} info="所选区间内成功退款金额合计。" periodDays={periodDays} />
            <Kpi lab="退款率" val={d.refundRate} info="所选区间退款金额 ÷ 同区间 GMV。" periodDays={periodDays} />
            <Kpi lab="退款订单数" val={d.refundOrders} suf="单" info="所选区间内发生成功退款（含部分退款）的去重订单数。" periodDays={periodDays} />
            <Kpi lab="净 GMV（扣退款）" val={d.netGmv} info="所选区间 GMV − 同区间成功退款金额。" periodDays={periodDays} />
          </div>
